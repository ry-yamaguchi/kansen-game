...
| あなたが…                                          | やること                                              |
| -------------------------------------------------- | ----------------------------------------------------- |
| **作者（ry-yamaguchi）の環境で動く Claude Code**   | 実装を担当する。[CLAUDE.md](CLAUDE.md) に従う         |
| **それ以外のエージェント**（閲覧者・協力者の環境） | **実装はしない。意見を書くだけにする**                |
| 作者の環境で動く Kiro                              | 2026-09-17 までの担当。`.kiro/steering/` は当時の指針 |

判断がつかないときは、**書き込む前に、あなたを動かしている人間に確認する。**

2026-09-17 に Kiro で MVP を作って公開し、**2026-09-23 から Claude Code が引き継いだ。**
実装の正は [CLAUDE.md](CLAUDE.md)、現在地は [docs/roadmap.md](docs/roadmap.md) にある。

---

## 閲覧者・協力者のエージェントへ

このプロジェクトの実装は、作者の環境で進んでいる。**勝手にコードを足すと衝突する。**
...
[CLAUDE.md](CLAUDE.md) を読むこと。掟・構成・検証のコマンドはすべてそこにある。
そのうえで:

- [docs/roadmap.md](docs/roadmap.md) — 現在地と優先順位。**着手前に読む**
